...
def class_agnostic_nms(box_scores, box_preds, nms_config, score_thresh=None):
    ...
    if box_scores.shape[0] > 0:
        ...
        # Try to get NMS from utils, otherwise use local CPU fallback
        nms_func = getattr(iou3d_nms_utils, nms_config.NMS_TYPE, None)
        
        if nms_func is None:
             # CPU implementation using torchvision (axis-aligned approximation)
             def nms_cpu(boxes, scores, thresh, **kwargs):
        ...
        keep_idx, selected_scores = nms_func(
